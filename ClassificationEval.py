...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 12 11:37:55 2026
...
#Adding some noise ro simulate random measurement error
np.random.seed(42)
...
#loading original and noisy datasets into a dataframe for comparision and visualization
...
#Visualizing original and noisy datasets
plt.hist(df[feature_names[5]],bins=20,alpha=0.7,color='blue',label='original')
plt.title('Original feature distribution')
plt.xlabel(feature_names[5])
plt.ylabel('Frequency')

plt.hist(df_noisy[feature_names[5]],bins=20,alpha=0.7,color='blue',label='original')
plt.title('noisy feature distribution')
plt.xlabel(feature_names[5])
plt.ylabel('Frequency')

#plotting the two features together to get a sense of their differences.
...
#Scatter plot to know the correlation
plt.figure(figsize=(12, 6))
...
#Checking if we are overfitting
...
#knn is showing some signs of overfitting while svm is in ideal condition
